...
**Test task for [ML4Sci — PINNDE Project](https://ml4sci.org/)**  
...
- **Model:** MLP with (z, ξ) input and scalar x output  
- **Physics loss:** ODE residual at collocation points  
- **IC loss:** enforce x(0)=0.7 and ẋ(0)=1.2 across ξ grid  
- **Fourier features:** learnable sinusoidal embedding of z for sharper oscillations  
- **Optimizer:** Adam + cosine LR schedule

## Results

Numbers below come from the included notebook run (device reported as CUDA).  
...
Animated sweep of the learned solution as ξ varies.
...
## Project Context (PINNDE)

This task mirrors the PINNDE conditioning idea:  
